...
- **💾 문서 관리**: 생성된 자기소개서 저장, 편집, 다운로드 기능
- **👤 사용자 프로필**: Google 계정 연동 및 기본 정보 관리
...
```
[사용자 입력] → [크롤링 엔진] → [데이터 가공] → [AI 분석] → [문서 생성] → [사용자 저장소]
```
...
| `/api/user-letters/` | GET/POST | 사용자 문서 관리 |
| `/api/user-profile/` | GET/PUT | 사용자 프로필 관리 |

### 외부 API 통합
- **Groq API**: 텍스트 생성 및 분석
- **Google OAuth API**: 사용자 인증

## 👤 사용자 흐름

### 회원가입 및 로그인
```
[첫 방문] → [Google 로그인] → [프로필 설정] → [대시보드]
```

### 자기소개서 생성
```
[채용공고 URL 입력] → [기업정보 확인] → [자기소개서 생성] → [검토 및 편집] → [저장/다운로드]
```
...
### 배포 환경
- **Render.com**: `production` 브랜치 푸시 시 자동 배포
- **환경 전환**: 개발/배포 환경 간 전환 스크립트 제공

## 🚢 CI/CD 파이프라인

### GitHub Actions 구성
```
[코드 푸시] → [테스트] → [빌드] → [배포(production 브랜치만)]
```

### 브랜치 전략
- **develop**: 개발 및 테스트용 (배포되지 않음)
