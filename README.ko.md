...
<div align="center">
  <img src="static_prod/images/logo.png" alt="CVFactory 로고" style="width:200px; height:auto;"/>
  <br>
...
## 📖 개요
CVFactory는 구직자를 위한 맞춤형 자기소개서 자동 생성 시스템입니다. 채용 공고와 지원자 정보를 기반으로 AI가 개인화된 자기소개서를 생성합니다. 시스템은 채용 공고와 기업 정보를 분석하여 생성된 문서에서 관련 기술과 경험을 강조합니다. 자기소개서를 하나하나 쓸 시간이 없는 구직자들을 위해 개발했습니다.

## 🌐 라이브 데모

실제 작동하는 애플리케이션은 **[cvfactory.dev](https://cvfactory.dev)** 에서 확인하실 수 있습니다.

CVFactory의 모든 기능을 직접 체험하고 나만의 맞춤형 자기소개서를 몇 분 안에 생성해보세요!
...
1. **`develop` 브랜치**: 개발 작업용 통합 브랜치
   - 모든 일상적인 개발 작업이 여기서 이루어짐
   - 테스트 및 통합에 사용
   - 이 브랜치에 푸시해도 파이프라인이 트리거되지 않음

2. **`production` 브랜치**: 프로덕션 준비가 완료된 코드만 포함
