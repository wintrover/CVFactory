...
# 애플리케이션 실행 (ASGI 사용)
CMD ["sh", "-c", "python manage.py collectstatic --noinput ; python manage.py makemigrations ; python manage.py migrate ; gunicorn --chdir /app cvfactory_project.asgi:application -k uvicorn.workers.UvicornWorker --bind 0.0.0.0:${PORT:-8000}"] 